...
class Keypad(object):
    ...
    def getKey(self):
        ...
        if (
            self.getKeys()
            and self.key[0].stateChanged
            and (self.key[0].kstate == self.key[0].PRESSED)
        ):
        ...
        for pin_c in self.colPins:
            ...
            for r in self.rowPins:  # keypress is active low so invert to high.
                self.bitMap[self.rowPins.index(r)] = self.bitWrite(
                    self.bitMap[self.rowPins.index(r)],
                    self.colPins.index(pin_c),
                    not GPIO.input(r),
                )
    ...
    def nextKeyState(self, idx, button):
        ...
        if self.key[idx].kstate == kk.IDLE:
            ...
            if (
                time.time() - self.holdTimer
            ) > self.holdTime * 0.001:  # Waiting for a key HOLD...
